<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="ExcuseOverdueTasks.aspx.cs" Inherits="QProcess.ExcuseOverdueTasks" MasterPageFile="OverdueTasksChargeMonitor.master" %>
    ...
    <script type="text/javascript">
        $(document).ready(function () {
           ...
           // currentDate.setDate(currentDate.getDate() - 1);
            ...
            $.ajax({
                url: "/DataService.asmx/GetAllActiveEmployees",
                type: "POST",
                data: {},
                async: false,
        ...
        {
            syncPost("GetOverdueTasksExcuse", JSON.stringify({ startDate: $("#txtStartDate").val(), endDate: $("#txtEndDate").val(), employee: $("#ddlPriorityEmployees option:selected").text() }));
            $("#tblScheduleReport").html(syncPostResult);
        }
        function ExcuseOverDueTask(id, dueDate)
        {
            //alert(employee + " " + scheduledDate)
            syncPost("ExcuseOverdueTask", JSON.stringify({ id: id ,dueDate:dueDate}));
            GetOverdueTasks();
        }
...
<asp:Content ID="Content3" runat="server" ContentPlaceHolderID="MainContent">
  ...
  <select id="ddlPriorityEmployees">
   ...
   <div style="margin-left: 10px; float: left;">
     ...
     <input id="btnGo" style="width: 40px; height: 25px; text-align: center; vertical-align: top;"  type="button" value="Go" onclick="GetOverdueTasks()" />
    </div>
 </div>
    ...
    <table id="tblScheduleReport" style="width:1100px;">
