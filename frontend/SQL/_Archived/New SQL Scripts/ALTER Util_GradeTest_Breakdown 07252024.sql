USE [PHIProcess]
GO
/****** Object:  StoredProcedure [dbo].[Util_GradeTest_Breakdown]    Script Date: 7/25/2024 10:33:11 AM ******/
SET ANSI_NULLS ON
GO
...
BEGIN
		...
		**********************************************************************
		...
		UNCOMMENT BELOW ONCE SECTIONS SECTION IS AVAILABLE IN BREAKDOWN UI

		**********************************************************************
		**********************************************************************
		**********************************************************************
		*/
		select r.Name ReportName, tt.Description SectionName, tt.DisplayOrder, tt.IsDeleted
		from qstatus_report r
				inner join qstatus_groupreport gr
						on r.id = gr.reportid
				inner join qcheck_groupmembership gm
		...
		SELECT *, 
			CASE
				WHEN name LIKE '%Total Tasks%' THEN '12 (or 13)'
				WHEN name LIKE '%Calendar Tasks%' THEN '10'
				WHEN name LIKE '%Controlled Tasks%' THEN '4'
				WHEN name LIKE '%Report Tasks%' THEN '3'
				WHEN name LIKE '%Priority Tasks%' THEN '4'
				WHEN name LIKE '%History Tasks%' THEN '3'
				WHEN name LIKE '%Change Requests%' THEN '3'
				WHEN name LIKE '%Timeline Tasks%' THEN '0'
				WHEN name LIKE '%Group%' THEN 'Sample ' + @Prefix
				WHEN name LIKE '%Help%' THEN 'Every task needs to be assigned to someone responsible for completing the task.'
				WHEN name LIKE '%Search%' THEN 'Choose AND logic if you only want results that have all the search text criteria. Ex) "NNN major" would only return items that have both NNN and major in the result.'
			END ExpectedAnswer
		from qcheck_checklists
		where owner = @testerUserID
				and CreateDate > @TestDate
		...
		IF(SELECT isnull(count(id), 0)
				...
				where owner = @testerUserID
						and CreateDate > @TestDate
						and (name like @Prefix + ' Controlled Tasks%4') --4
		) = 1
				SET @writtenSection = @writtenSection + 1
		...
		IF(SELECT isnull(count(id), 0)
				...
				where owner = @testerUserID
						and CreateDate > @TestDate
						and (name like @Prefix + ' Report Tasks%3')--3
		) = 1
				SET @writtenSection = @writtenSection + 1
		...
		IF(SELECT isnull(count(id), 0)
				...
				where owner = @testerUserID
						and CreateDate > @TestDate
						and (name like @Prefix + ' Priority Tasks%4')--4
		) = 1
				SET @writtenSection = @writtenSection + 1
		...
		IF(SELECT isnull(count(id), 0)
				...
				where owner = @testerUserID
						and CreateDate > @TestDate
						and (name like @Prefix + ' History Tasks%3')--3
		) = 1
				SET @writtenSection = @writtenSection + 1
		...
		IF(SELECT isnull(count(id), 0)
				...
				where owner = @testerUserID
						and CreateDate > @TestDate
						and (name like @Prefix + ' Group%Sample%Process%')
		) = 1
				SET @writtenSection = @writtenSection + 1
