...
namespace QProcess
{
    ...
    public partial class Priorities : SessionPage
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    {
                        SetId = PrioritySets.FirstOrDefault(pg => pg.LastPrioritySet == 1) != null 
                            ? PrioritySets.First(pg => pg.LastPrioritySet == 1).Id 
                            : PrioritySets.First().Id;
                    }
                }
                ...
                //Info.LastChecked = QProcess.Session.CurrentSession.UserTimeZone.GetLocalTimeNow();
                //}
                //else
                //{
                //SetLockedByMe = true;
        ...
        protected void Redirect()
        {
            using (var cmd = new CmdObj("PriorityListSet_Redirect"))
            {
                ...
                else
                {
                    if (controller)
                    {
                        url = "MyStatus.aspx?reportID=" + reportID.ToString();
                    }
                    else
                    {
                        url = "MyInbox.aspx?reportID=" + reportID.ToString();
                    }
