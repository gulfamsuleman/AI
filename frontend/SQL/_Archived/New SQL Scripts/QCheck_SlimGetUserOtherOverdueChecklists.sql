...
/****** Object:  StoredProcedure [dbo].[QCheck_SlimGetUserOtherOverdueChecklists]   Script Date: 9/27/2024 2:19:23 PM ******/

/* OG (9/27/24 req by Ken Croft): 
	Get all overdue tasks NOT within the filtered date range:
		1. Completed date is NULL
		2. DueTime is before the first date filter
		3. DueTime is before today and after the second date filter */
...
CREATE PROC [dbo].[QCheck_SlimGetUserOtherOverdueChecklists](
	@UserID int,
...
BEGIN
	...
	FROM
	 ...
	 WHERE
		...
		AND @instanceID = 0
		AND (a.DueTime < @startDate OR (a.DueTime < GETDATE() AND a.DueTime > @endDate))
