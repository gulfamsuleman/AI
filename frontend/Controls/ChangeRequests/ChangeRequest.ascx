<%@ Control Language="C#" AutoEventWireup="true" CodeBehind="ChangeRequest.ascx.cs" Inherits="QProcess.Controls.Controls_ChangeRequests_ChangeRequest" %>
...
<tr class="change-request change-request-header change-request-line" data-request-checklist-id="<%:ChangeRequest.ChecklistId%>" data-request-id="<%:ChangeRequest.ID%>" data-email="<%:ChangeRequest.Email %>">
    <td class="control-table-border cr-header-date">
        <div data-role="wrap" class="cr-date-nowrap nowrap">
        <i class="fa fa-plus-square-o proxy-expand" data-role="expand-indicator" data-request-id="<%:ChangeRequest.ID%>"></i>
        <a data-role="expand"></a>
        <span class="hidden data-label">Date</span>
        <span style="word-break:break-all;"><%:ChangeRequest.RequestDate.ToShortDateString() %></span>
    </div>

</td>
<td class="control-table-border cr-header-requestor">
    <div data-role="wrap" class="cr-requestor-nowrap">
        <%if (CanApprove)
            { %>
            <img src="/images/ChatBubbleC.svg" style="width: 20px; cursor:pointer;"
                data-request-checklist-name="<%:ChangeRequest.ChecklistName%>"
                data-email="<%:ChangeRequest.Email %>" 
                data-role="quick-comments" />
        <% } %>
        <span class="hidden data-label">Requestor</span>
        <span style="word-break:break-word;"><%: ChangeRequest.FullName %></span>
    </div>
</td>
    <td class="control-table-border cr-header-task">
        <div data-role="wrap" class="cr-task-nowrap">
            <span class="hidden data-label">Task</span>
            <span><%= ChangRequestFormattedName() %></span>
        </div>
    </td>
    <td class="control-table-border cr-header-overview">
        <div data-role="wrap" class="cr-overview-nowrap">
            <span class="hidden data-label">Request Type / Comments</span>
            <span><%= ChangeRequestOverview() %></span>
        </div>
    </td>
    <td class="control-table-border cr-header-actions">
        <div class="cr-actions-nowrap">
            <span class="hidden data-label">Actions</span>
            <%if (CanApprove)
                { %>
            <a data-role="approve" title="Approve" class="btn-border btn-default btn-sm">Appr.</a>
            <a data-role="deny" title="Deny" class="btn-border btn-default btn-sm">Deny</a>
            <% } %>

            <a data-role="cancel" title="Cancel" class="btn-border btn-default btn-sm">Cancel</a>
            <a data-role="change-view" title="Change/View" class="btn-border btn-default btn-sm">Change</a>
        </div>
    </td>
</tr>
<tr class="change-request-line" data-request-checklist-id="<%:ChangeRequest.ChecklistId%>" data-request-id="<%:ChangeRequest.ID%>" data-email="<%:ChangeRequest.Email %>">
    <td colspan="5">
        <div class="change-request collapsible-item" data-collapse-state="collapsed" data-request-checklist-id="<%:ChangeRequest.ChecklistId%>" data-email="<%:ChangeRequest.Email %>">
            <i class="fa fa-plus-square-o proxy-target" style="display: none;" data-request-id="<%:ChangeRequest.ID%>" data-role="expand-indicator" data-collapse-target="true"></i>
            <div class="change-request-inner-header">
            ...
            <div class="change-request-body collapsed">
                ...
                <div class="reply-section">
                    ...
                    <br />
                    <input type="button" value="Send Reply Email" data-role="email" class="btn btn-default" />
                    ...
                    <table class="previous-table table table-condensed table-responsive">
                        ...
                        <tbody>
                            <asp:Repeater ID="rptHistory" runat="server">
                                <ItemTemplate>
                                    <tr>
                                        <td valign="top" nowrap><%# DataBinder.Eval(Container, "DataItem.DueTime") %>&nbsp;</td>
                                        <td><%# DataBinder.Eval(Container, "DataItem.Comment") %>&nbsp;</td>
                                        <td valign="top" nowrap><%# DataBinder.Eval(Container, "DataItem.RequestedBy") %>&nbsp;</td>
                                        <td valign="top" nowrap><%# DataBinder.Eval(Container, "DataItem.RequestDate") %>&nbsp;</td>
                                    </tr>
                                </ItemTemplate>
                            </asp:Repeater>
                        </tbody>
