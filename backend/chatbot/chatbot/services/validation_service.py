...
class ValidationService:
    ...
    def validate_task_name(task_name: str) -> None:
        ...
        # Allow apostrophes (') and commas (,) in task names
        invalid_chars = ['<', '>', '"', '\\', '|', '\x00']
    ...
    def validate_assignees(assignees: str) -> List[str]:
        """
        Validate and parse assignees list against QCheck_Groups table.
        ...
        # Validate each assignee name against QCheck_Groups table
        validated_assignees = []
        for assignee in assignee_list:
            ...
            # Validate against QCheck_Groups table
            try:
                group_exists, found_name, similar_groups = DatabaseService.validate_group_exists(assignee)
                if group_exists:
                    validated_assignees.append(found_name)
                else:
                    if similar_groups:
                        suggestions = ", ".join(similar_groups[:3])
                        raise ValidationError(
                            f'"{assignee}" is not found in QCheck_Groups. Did you mean one of these: {suggestions}?'
                        )
                    else:
                        raise ValidationError(
                            f'"{assignee}" is not found in QCheck_Groups. Please use a valid group name.'
                        )
            except Exception as e:
                logger.error(f"Error validating assignee '{assignee}' against QCheck_Groups: {e}")
                raise ValidationError(f"Unable to validate assignee '{assignee}'. Please try again.")
