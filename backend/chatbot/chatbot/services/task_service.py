            ...
            try:
                ...
                if not is_valid:
                    ...
                    if SessionService._is_name_clarification_response(user_message, params):
                        ...
                        resolved_params = SessionService._handle_name_clarification_response(user_message, params)
                        if resolved_params:
                ...
                if params.get('_alert_required') or params.get('IsAlert'):
                    ...
                    if params.get('DueTime'):
                        ...
                        if ':' in due_time_str:
                            ...
                            alert_params['_alert_due_time_hours'] = float(hours)
                        else:
                            alert_params['_alert_due_time_hours'] = 9.0
                    else:
                        alert_params['_alert_due_time_hours'] = 9.0
                    
                    logger.info(f"Alert parameters from task service: {params.get('_alert_required')}, {alert_params['_alert_recipient']}, {alert_params['_alert_condition']}, {alert_params['_alert_type']}")
                    
                    # Create the alert
                    try:
                        alert_success = DatabaseService.create_alert_for_task(new_instance_id, alert_params)
                        if alert_success:
                            logger.info(f"Successfully created alert for task {new_instance_id}")
                        else:
                            logger.warning(f"Failed to create alert for task {new_instance_id}")
                    except Exception as alert_error:
                        logger.error(f"Error creating alert for task {new_instance_id}: {alert_error}")
    ...
    def _convert_time_to_int(self, time_str: str) -> int:
        ...
        try:
            ...
            if len(time_parts) == 2:
                ...
                return hours  # Return just the hours as expected by database
            else:
                return 19  # Default to 7 PM (hour 19)
        except:
            return 19  # Default to 7 PM (hour 19)
