var calendarModule = (function () {
    ...
    $().ready(function () {
        ...
        datepicker.val(calendarPage.queryDate);
        ...
        if (filterMask) {
            ...
            //defaults - don't show alert emails or completed tasks
            toggleDisplay($("input[data-action='toggleAlerts']"), false);
            toggleDisplay($("input[data-action='toggleCompleted']"), false);
        }
    });
    ...
    function toggleDisplay(target, forceValue) {
        ...
        if (isChecked) $(selector).removeClass(dataAction == "toggleControl" ? "hideControl" : "hideFilter");
        else $(selector).addClass(dataAction == "toggleControl" ? "hideControl" : "hideFilter");

        if ($("#overdue-panel a.bold:not(.hideFilter, .hideControl)").length == 0)
            $("#overdue-panel").hide();
        else
            $("#overdue-panel").show();
    ...
    function updateSelection(date) {
        ...
        var href = "MobileCalendar.aspx?date=" + date;
        if (assigneeId)
    ...
    function updateAssignee() {
        ...
        window.location.href = "MobileCalendar.aspx?date=" + datepicker.val() + "&assigneeId=" + assigneeId + "&showStatus=" + showStatus + "&filterMask=" + (filterMask ? filterMask : 103);
    }
    ...
    self.itemDrop = function (event) {
        ...
        var dragItem = $("div.dayofweekDiv a.activeTask[data-unique-id=" + acId + "], div.dayofweekDiv a.overdueTask[data-unique-id=" + acId + "]");
